...
const FeedComponent = () => {
  ...
  useEffect(() => {
    const fetchRSS = async () => {
      try {
        //const CORS_PROXY = "https://proxy.cors.sh/"; // Use a CORS proxy if needed
        const response = await fetch("https://proxy.cors.sh/https://puberkalom.com/feed", {
          headers: {
            "x-cors-api-key": "temp_d58eed24f5627cce8956e1a020a5241c",
          },
        });
        const textResponse = await response.text();

        //console.log("Response:", textResponse); // Log the raw response for debugging
        ...
        const itemsArray = Array.from(xml.getElementsByTagName("item")).map(
          (item) => {
            const getValue = (tag) => {
